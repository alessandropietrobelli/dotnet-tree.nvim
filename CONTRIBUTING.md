...
| `lua/dotnet-tree/parser/` | `sln`, `slnx`, `csproj`, `cpm`, `props` (the `Directory.Build.props` above a project), `launch_settings` (the launch profile `d` starts with), the shared `xml` reader, and discovery |
